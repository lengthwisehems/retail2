...
STOQ_DEMAND_API_TOKEN = ""  # Set via env RR_DEMAND_API_TOKEN, RESTOCK_ROCKET_DEMAND_TOKEN, or STOQ_API_TOKEN
STOQ_TOKEN_ENV_VARS = ["RR_DEMAND_API_TOKEN", "RESTOCK_ROCKET_DEMAND_TOKEN", "STOQ_API_TOKEN"]
...
def _extract_js_object(text: str, start_pos: int) -> Optional[str]:
    """Extract a complete JS object/array starting at start_pos using bracket matching."""
    depth = 0
    in_string = False
    string_char = None
    escape_next = False
...
def _extract_js_assignment(text: str, var_name: str) -> Optional[str]:
    """Return the last non-null JS assignment value for var_name as a raw string."""
    pattern = re.compile(r"\b" + re.escape(var_name) + r"\s*=\s*(?!null\s*;)")
    last_match = None
    ...
    if start >= len(text) or text[start] not in ("{", "["):
        return None
    return _extract_js_object(text, start)


def _js_to_json(text: str) -> str:
    """Best-effort conversion of JavaScript object notation to JSON."""
    # Remove single-line comments
    text = re.sub(r"//[^\n]*", "", text)
    # Remove trailing commas before } or ]
    text = re.sub(r",\s*([}\]])", r"\1", text)
    # Quote unquoted object keys (identifier immediately followed by colon,
    # preceded by { or , and optional whitespace/newlines)
    text = re.sub(
        r'([{,\[]\s*\n?\s*)([a-zA-Z_$][a-zA-Z0-9_$]*)\s*:',
        lambda m: f'{m.group(1)}"{m.group(2)}":',
        text,
    )
    # Also handle keys at start of expression that weren't caught above
    text = re.sub(
...
def extract_restock_variants(product: Dict[str, Any]) -> List[Dict[str, Any]]:
    variants: List[Dict[str, Any]] = []
    seen_ids: Set[Any] = set()
    raw_variants = product.pop("variants", None) or []
    if isinstance(raw_variants, list):
        for item in raw_variants:
            ...
            if "quantity" in variant and "quantityAvailable" not in variant:
                variant["quantityAvailable"] = variant["quantity"]
...
def _resolve_stoq_token(settings: Optional[Dict[str, Any]], logger: logging.Logger) -> Optional[str]:
    if settings:
        for key in ["external_api_key", "demand_api_key", "api_token", "apiKey"]:
            token = settings.get(key)
            if token:
                logger.info("STOQ token from RR settings key '%s'", key)
                return str(token)
    if STOQ_DEMAND_API_TOKEN:
        return STOQ_DEMAND_API_TOKEN
    for env_key in STOQ_TOKEN_ENV_VARS:
        val = os.environ.get(env_key)
        if val:
            logger.info("STOQ token from env %s", env_key)
            return val
    logger.warning("No STOQ demand API token found; demand data will be skipped")
    return None
...
def fetch_restockrocket_rows(
    ...
    stoq_token = _resolve_stoq_token(settings, logger)
...
def fetch_restock_data(
    ...
    for handle in all_handles:
        ...
        if isinstance(preorder_data, dict):
            for k, v in preorder_data.items():
                product_copy[f"restock_preorder.{k}"] = v
...
def main() -> None:
    ...
    restock_rows, restock_tag_columns = fetch_restock_data(session, logger)
