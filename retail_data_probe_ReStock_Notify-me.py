...
RESTOCK_SHOP_ID = 35914
RESTOCK_PDP_BASE = "https://rudesdenim.com/products"
...
def _extract_js_object(text: str, search_from: int = 0) -> Optional[str]:
    """Extract a complete JS object literal starting at the first { found from search_from."""
...
def _js_to_json(js_text: str) -> str:
    ...
    # Quote unquoted identifier keys (word chars followed by optional space + colon)
    js_text = re.sub(
...
def _parse_restock_script(script_text: str) -> Dict[str, Any]:
    """Parse all _ReStockConfig fields from a PDP script block."""
    result: Dict[str, Any] = {}

    # --- product object ---
    prod_match = re.search(r"_ReStockConfig\.product\s*=\s*", script_text)
    if prod_match:
        raw_block = _extract_js_object(script_text, prod_match.end())
        if raw_block:
            try:
                result["product"] = json.loads(_js_to_json(raw_block))
            except (ValueError, KeyError):
                pass

    # --- templateName ---
    tn = re.search(r'_ReStockConfig\.templateName\s*=\s*"([^"]*)";', script_text)
    if tn:
        result["templateName"] = tn.group(1)

    # --- marketHandle ---
    mh = re.search(r'_ReStockConfig\.marketHandle\s*=\s*"([^"]*)";', script_text)
    if mh:
        result["marketHandle"] = mh.group(1)

    # --- marketId ---
    mi = re.search(r"_ReStockConfig\.marketId\s*=\s*(\d+);", script_text)
    if mi:
        result["marketId"] = int(mi.group(1))

    # --- isB2BCustomer ---
    b2b = re.search(r"_ReStockConfig\.isB2BCustomer\s*=\s*(null|true|false);", script_text)
    if b2b:
        val = b2b.group(1)
        result["isB2BCustomer"] = None if val == "null" else val == "true"

    # --- currentLocationId ---
    loc = re.search(r"_ReStockConfig\.currentLocationId\s*=\s*(null|\d+);", script_text)
    if loc:
        val = loc.group(1)
        result["currentLocationId"] = None if val == "null" else int(val)

    def _extract_json_var(var_name: str) -> Optional[Any]:
        """Extract a JSON-assigned variable like: varName = {...};"""
        positions = [m.start() for m in re.finditer(re.escape(var_name) + r"\s*=\s*[^n]", script_text)]
        if not positions:
            return None
        for pos in reversed(positions):
            eq_match = re.search(r"=\s*", script_text[pos:])
            if not eq_match:
                continue
            value_start = pos + eq_match.end()
            ch = script_text[value_start] if value_start < len(script_text) else ""
            if ch == "{":
                raw_block = _extract_js_object(script_text, value_start)
                if raw_block:
                    try:
    ...
    result["sdkConfig"] = _extract_json_var("metafieldConfigData")
    result["preOrderConfig"] = _extract_json_var("metafieldPreOrderData")
    result["wishlistConfig"] = _extract_json_var("metafieldWishlistData")

    return result


def _flatten_restock_meta(prefix: str, obj: Any) -> Dict[str, Any]:
    """Flatten a nested metafield dict into dot-separated column keys."""
    out: Dict[str, Any] = {}
    ...
    for k, v in obj.items():
        ...
        if isinstance(v, dict):
            out.update(_flatten_restock_meta(full_key, v))
...
def fetch_restock_data(
    ...
    """Fetch ReStock / notify-me.app data from each product PDP page.

    Parses _ReStockConfig injected by the restock-sdk-loader and returns
    one row per SKU/variant with all available fields, plus flattened
    metafield data (sdkConfig, preOrderConfig, wishlistConfig).
    """
    base = RESTOCK_PDP_BASE.rstrip("/")

    # Discover handles from collection products.json if not provided
    if handles is None:
        handles = []
        for products_json_url in build_products_json_urls():
            page = 1
            while True:
                try:
                    resp = session.get(
                    ...
                    logger.warning("ReStock handle discovery request failed: %s", exc)
                    break
                if not resp.ok:
                    logger.warning(
                        "ReStock handle discovery returned status %s", resp.status_code
                    )
                    break
                try:
                    payload = resp.json()
                ...
                if not isinstance(products, list) or not products:
                    break
                for p in products:
                    h = p.get("handle") if isinstance(p, dict) else None
    ...
    logger.info("ReStock: fetching PDP pages for %s handles", len(handles))

    rows: List[Dict[str, Any]] = []

    for handle in handles:
        pdp_url = f"{base}/{handle}"
        try:
            resp = session.get(pdp_url, timeout=REQUEST_TIMEOUT, verify=False)
        except requests.RequestException as exc:
            logger.warning("ReStock PDP fetch failed for %s: %s", handle, exc)
            continue
        if not resp.ok:
            logger.warning("ReStock PDP %s returned status %s", handle, resp.status_code)
            continue
        ...
        for tag in soup.find_all("script"):
            text = tag.string or ""
            if "_ReStockConfig" in text:
                script_text = text
                break

        if not script_text:
            logger.debug("No _ReStockConfig found in PDP HTML for handle=%s", handle)
            continue

        parsed = _parse_restock_script(script_text)
        product_data: Dict[str, Any] = dict(parsed.get("product") or {})
        sdk_config = parsed.get("sdkConfig") or {}
        pre_order_config = parsed.get("preOrderConfig") or {}
        wishlist_config = parsed.get("wishlistConfig") or {}

        # Flatten metafield configs once per product (reused for each variant row)
        meta_flat: Dict[str, Any] = {}
        meta_flat.update(_flatten_restock_meta("metafield.sdkConfig", sdk_config))
        meta_flat.update(_flatten_restock_meta("metafield.preOrderConfig", pre_order_config))
        meta_flat.update(_flatten_restock_meta("metafield.wishlistConfig", wishlist_config))

        # Top-level ReStock context fields
        context_flat: Dict[str, Any] = {
            "restock.templateName": parsed.get("templateName"),
            "restock.marketHandle": parsed.get("marketHandle"),
            "restock.marketId": parsed.get("marketId"),
            "restock.isB2BCustomer": parsed.get("isB2BCustomer"),
            "restock.currentLocationId": parsed.get("currentLocationId"),
            "restock.shop_id": sdk_config.get("shop_id"),
            "restock.app_enabled": sdk_config.get("app_enabled"),
            "restock.is_back_in_stock_active": sdk_config.get("is_back_in_stock_active"),
            "restock.is_service_active_low_stock": (sdk_config.get("low_stock") or {}).get(
                "is_service_active"
            ),
            "restock.is_suspended": sdk_config.get("is_suspended"),
            "restock.back_in_stock_product_selection_mode": sdk_config.get(
                "back_in_stock_product_selection_mode"
            ),
            "restock.pre_order_is_service_active": pre_order_config.get("is_service_active"),
            "restock.wishlist_is_service_active": wishlist_config.get("is_service_active"),
        }

        # Product-level fields (pop variants before flattening)
        variants_raw = product_data.pop("variants", [])
        images_raw = product_data.get("images") or []
        featured_image = product_data.get("featured_image") or (
            images_raw[0] if images_raw else None
        )

        product_flat: Dict[str, Any] = {
            "product.id": product_data.get("id"),
            "product.handle": product_data.get("handle") or handle,
            "product.title": product_data.get("title"),
            "product.available": product_data.get("available"),
            "product.price": product_data.get("price"),
            "product.compare_at_price": product_data.get("compare_at_price"),
            "product.vendor": product_data.get("vendor"),
            "product.featured_image": featured_image,
            "product.images": json.dumps(images_raw, ensure_ascii=False) if images_raw else None,
            "product.images[0].src": featured_image,
            "product.onlineStoreUrl": pdp_url,
            "product.selected_or_first_available_variant_id": product_data.get(
                "selected_or_first_available_variant_id"
            ),
            "product.selling_plan_groups": json.dumps(
                product_data.get("selling_plan_groups") or [], ensure_ascii=False
            ),
        }
        ...
        for variant in variants_raw:
            ...
            variant_row.update(context_flat)
            variant_row.update(product_flat)
            variant_row["variant.id"] = variant.get("id")
            variant_row["variant.title"] = variant.get("title")
            variant_row["variant.sku"] = variant.get("sku")
            variant_row["variant.available"] = variant.get("available")
            variant_row["variant.quantity"] = variant.get("quantity")
            variant_row["variant.quantityAvailable"] = variant.get("quantity")
            variant_row["variant.price"] = variant.get("price")
            variant_row["variant.compare_at_price"] = variant.get("compare_at_price")
            variant_row["variant.inventory_management"] = variant.get("inventory_management")
            variant_row["variant.featured_image"] = variant.get("featured_image")
            variant_row["variant.name"] = variant.get("name")
            variant_row.update(meta_flat)
...
def export_workbook(
    ...
    *,
    ...
    restock_priority_columns: Optional[Sequence[str]] = None,
) -> Path:
    workbook = Workbook()
    ...
    restock_sheet = workbook.create_sheet("ReStock")
    restock_columns = (
        build_column_order(
            restock_rows, extra_priority=restock_priority_columns
        )
        if restock_rows
        else list(COLUMN_ORDER_BASE)
    )
    write_sheet(restock_sheet, restock_rows, column_order=restock_columns)
    group_tag_columns(restock_sheet, restock_columns)
...
def main() -> None:
    ...
    restock_handles = [
        str(row.get("product.handle") or "").strip()
        for row in json_rows
        if isinstance(row, dict) and str(row.get("product.handle") or "").strip()
    ]
    restock_handles = list(dict.fromkeys(restock_handles))
    restock_rows, restock_tag_columns = fetch_restock_data(session, logger, handles=restock_handles or None)
    output_path = export_workbook(
        ...
        restock_priority_columns=restock_tag_columns,
    )
