"""Abrand Jeans monthly scraper with HTML measurement fallback."""
...
LOG_PATH = BASE_DIR / "abrand_measurements_run.log"
FALLBACK_LOG_PATH = OUTPUT_DIR / "abrand_measurements_run.log"
...
def fetch_measurements_from_html(handle: str) -> Dict[str, str]:
    ...
    html_headers = {**DEFAULT_HEADERS, "Accept": "text/html,application/xhtml+xml"}
    for base in HOSTS:
        ...
        if response.status_code != 200:
            LOGGER.warning("Measurement fetch for %s returned %s", response.url, response.status_code)
            continue
        ...
        for li in soup.select("li"):
            ...
            if any(
                ...
                for keyword in ("inseam", "inleg", "inner leg")
            ):
                value = extract_measure(text, "Inseam", ["Inleg", "Inner Leg Length"])
...
def write_csv(rows: List[Dict[str, str]]) -> Path:
    ...
    output_path = OUTPUT_DIR / f"ABRAND_Measurements_{timestamp}.csv"
...
def main() -> None:
    LOGGER.info("Starting Abrand Jeans measurement scrape")
