"""Brand-agnostic probe that inspects Shopify collection feeds, Storefront APIs, and Rebuy."""
...
REBUY_CUSTOM_ID = "282563"
REBUY_BASE_URL = "https://rebuyengine.com/api/v1/custom/id"
REBUY_LIMIT = 250
...
# ---------------------------------------------------------------------------
# Rebuy field exclusions
# ---------------------------------------------------------------------------
REBUY_EXCLUDED_TOP_LEVEL: Set[str] = {
    "cache_info",
    "images",
    "media",
    "owner",
...
REBUY_EXCLUDED_IMAGE_SUBFIELDS: Set[str] = {
...
REBUY_EXCLUDED_CATEGORY_SUBFIELDS: Set[str] = {
    "admin_graphql_api_id",
    "full_name",
}

# Cache for resolved metaobject GIDs → human-readable names
_METAOBJECT_CACHE: Dict[str, str] = {}
...
def _query_metaobject_name(
    ...
    """Query the Shopify Storefront API to resolve a metaobject GID to a human-readable name."""
    if not endpoint:
        return None
    query = """
query ResolveMetaobject($id: ID!) {
  node(id: $id) {
    ... on Metaobject {
      type
      fields {
...
"""
    ...
    try:
        ...
        logger.debug("Metaobject query failed for %s: %s", gid, exc)
        return None
    if not response or not response.ok or not data:
        return None
    node = ((data.get("data") or {}).get("node") or {})
    if not node:
        return None
    fields = node.get("fields") or []
    for name_key in ("name", "title", "label", "value", "text"):
        for field in fields:
            if isinstance(field, dict) and field.get("key") == name_key:
                val = field.get("value")
                if val:
                    return str(val)
...
def resolve_metaobject_value(
    ...
    """If value contains Shopify metaobject GIDs, try to resolve them to human-readable names."""
    if raw_value is None:
        return raw_value
    str_value = str(raw_value).strip()
    if "gid://shopify/Metaobject/" not in str_value:
        return raw_value
    gids: List[str] = []
    try:
        parsed = json.loads(str_value)
        if isinstance(parsed, list):
            gids = [str(g) for g in parsed if "gid://shopify/Metaobject/" in str(g)]
        elif isinstance(parsed, str) and "gid://shopify/Metaobject/" in parsed:
            gids = [parsed]
    except (json.JSONDecodeError, ValueError):
        gids = re.findall(r"gid://shopify/Metaobject/\d+", str_value)
    if not gids:
        return raw_value
    resolved_names: List[str] = []
    for gid in gids:
        if gid in _METAOBJECT_CACHE:
            resolved_names.append(_METAOBJECT_CACHE[gid])
            continue
        name = _query_metaobject_name(gid, session, token, endpoint, logger)
        resolved = name if name else gid
        _METAOBJECT_CACHE[gid] = resolved
        resolved_names.append(resolved)
        time.sleep(0.05)
    return ", ".join(resolved_names) if resolved_names else raw_value


def apply_rebuy_metafield_columns(
    row: Dict[str, Any],
    metafields: Sequence[Any],
    prefix: str,
    session: requests.Session,
    token: Optional[str],
    endpoint: Optional[str],
    ...
    """Apply Rebuy metafield values to the row using the key as the column name.

    Column format: {prefix}.metafield.{key}
    Metaobject GID references are resolved to human-readable names where possible.
    """
    for mf in metafields:
        ...
        if not key or value in (None, ""):
            continue
        resolved = resolve_metaobject_value(value, session, token, endpoint, logger)
        row[f"{prefix}.metafield.{key}"] = resolved


def _finalize_rebuy_row(
    row: Dict[str, Any],
    product: Dict[str, Any],
    variant: Optional[Dict[str, Any]],
) -> None:
    """Finalize a Rebuy product/variant row, applying standard column normalization."""
    finalize_common_row(row, product, variant, source="json")
    # Remove any excluded fields that survived flattening
    for col in list(row.keys()):
        if (
            col.startswith("product.cache_info")
            or col.startswith("product.images[")
            or col.startswith("product.media")
            or col.startswith("product.owner")
        ):
            row.pop(col, None)
    for excluded_col in (
        "product.has_variants_that_requires_components",
        "product.category.admin_graphql_api_id",
        "product.category.full_name",
        "product.image.admin_graphql_api_id",
        "product.image.created_at",
        "product.image.id",
        ...
        row.pop(excluded_col, None)
    # Remove image.variant_ids flattened entries
    for col in list(row.keys()):
        if col.startswith("product.image.variant_ids"):
            row.pop(col, None)
    # Remove variant_gids.updated_at entries
    for col in list(row.keys()):
        if col.startswith("product.variant_gids") and col.endswith(".updated_at"):
            row.pop(col, None)


def fetch_rebuy_data(
    session: requests.Session, logger: logging.Logger
) -> Tuple[List[Dict[str, Any]], List[str]]:
    """Fetch all products and variants from the Rebuy custom endpoint.

    Paginates through all pages, extracts metafields (using the key as the
    column header), resolves metaobject GID references, and returns one row
    per variant.
    """
    if not REBUY_API_KEY or not REBUY_CUSTOM_ID:
        logger.info("Rebuy configuration missing; skipping Rebuy extraction")
        return [], []

    urls: List[str] = []
    if isinstance(COLLECTION_URL, (list, tuple)):
        urls = [u for u in COLLECTION_URL if u]
    elif isinstance(COLLECTION_URL, str) and COLLECTION_URL:
        urls = [COLLECTION_URL]
    if not urls:
        logger.info("No COLLECTION_URL configured; skipping Rebuy extraction")
        return [], []

    # Storefront token/endpoint for metaobject GID resolution
    sf_tokens = normalize_tokens(X_SHOPIFY_STOREFRONT_ACCESS_TOKEN)
    sf_token: Optional[str] = sf_tokens[0] if sf_tokens else None
    sf_endpoints = determine_graphql_endpoints()
    sf_endpoint: Optional[str] = sf_endpoints[0] if sf_endpoints else None
    ...
    for collection_url in urls:
        ...
        while True:
            ...
            api_url = f"{REBUY_BASE_URL}/{REBUY_CUSTOM_ID}"
            logger.info("Fetching Rebuy page %s for %s", page, collection_url)
            try:
                response = session.get(api_url, params=params, timeout=REQUEST_TIMEOUT, verify=False)
            except requests.RequestException as exc:
                logger.warning("Rebuy request failed on page %s: %s", page, exc)
                break
            if not response.ok:
                logger.warning(
                    "Rebuy returned status %s on page %s", response.status_code, page
                )
                break
            try:
                payload = response.json()
            except ValueError:
                logger.warning("Rebuy response on page %s was not valid JSON", page)
                break

            products = payload.get("data") or []
            if not isinstance(products, list) or not products:
                logger.info("Rebuy page %s returned no products; stopping pagination", page)
                break
            ...
            fetched_this_url += len(products)
            logger.info(
                "Rebuy page %s: %s products (fetched for url: %s / reported total: %s)",
                page, len(products), fetched_this_url, total_count,
            )
            if len(products) < REBUY_LIMIT:
                break
            if total_count and fetched_this_url >= total_count:
                break
            page += 1
            time.sleep(0.5)

    logger.info("Rebuy collected %s unique products total", len(all_products))
    if not all_products:
        return [], []
    ...
    for product in all_products:
        ...
        # Remove excluded top-level fields
        for excluded_key in REBUY_EXCLUDED_TOP_LEVEL:
            product_copy.pop(excluded_key, None)

        # Clean image sub-fields
        image = product_copy.get("image")
        if isinstance(image, dict):
            product_copy["image"] = {
                k: v for k, v in image.items() if k not in REBUY_EXCLUDED_IMAGE_SUBFIELDS
            }

        # Clean category sub-fields
        category = product_copy.get("category")
        if isinstance(category, dict):
            product_copy["category"] = {
                k: v for k, v in category.items() if k not in REBUY_EXCLUDED_CATEGORY_SUBFIELDS
            }

        # Remove updated_at from variant_gids entries
        variant_gids = product_copy.get("variant_gids")
        if isinstance(variant_gids, list):
            product_copy["variant_gids"] = [
                {k: v for k, v in entry.items() if k != "updated_at"}
                if isinstance(entry, dict) else entry
                for entry in variant_gids
            ]

        # Pop fields that need special handling
        product_metafields: List[Any] = product_copy.pop("metafields", []) or []
        if not isinstance(product_metafields, list):
            product_metafields = []

        variants_raw: List[Any] = product_copy.pop("variants", []) or []
        if not isinstance(variants_raw, list):
            variants_raw = []

        tags_raw = product_copy.pop("tags", []) or []
        ...
        # Derive image src for images[0].src compatibility
        image_src: Optional[str] = None
        img_field = product_copy.get("image")
        if isinstance(img_field, dict):
            image_src = img_field.get("src") or img_field.get("url")

        # Flatten product
        flat_product = flatten_record({"product": product_copy})
        base_row: Dict[str, Any] = dict(flat_product)
        ...
        # Map Rebuy 'link' field to onlineStoreUrl
        if "product.link" in base_row and "product.onlineStoreUrl" not in base_row:
            base_row["product.onlineStoreUrl"] = base_row["product.link"]

        # Apply product metafields (key as column, GIDs resolved)
        apply_rebuy_metafield_columns(
            base_row, product_metafields, "product",
            session, sf_token, sf_endpoint, logger,
        )

        tag_groups = group_tags_for_columns(tags)

        def attach_tag_groups(target_row: Dict[str, Any]) -> None:
            for col, tag_vals in tag_groups.items():
                target_row[col] = ", ".join(tag_vals)
                tag_group_counts[col] += 1

        if not variants_raw:
            row = dict(base_row)
            attach_tag_groups(row)
            _finalize_rebuy_row(row, product, None)
            rows.append(row)
            continue

        seen_variant_ids: Set[Any] = set()
        for variant in variants_raw:
            ...
            variant_copy = dict(variant)
            variant_metafields: List[Any] = variant_copy.pop("metafields", []) or []
            if not isinstance(variant_metafields, list):
                variant_metafields = []
            ...
            apply_rebuy_metafield_columns(
                row, variant_metafields, "variant",
                session, sf_token, sf_endpoint, logger,
            )

            attach_tag_groups(row)
            _finalize_rebuy_row(row, product, variant_copy)
            rows.append(row)
    ...
    all_columns = {key for row in rows for key in row.keys()}
    tag_group_columns = [col for col in all_columns if col.startswith("tags_group_")]
    tag_group_columns.sort(key=lambda col: (-tag_group_counts.get(col, 0), col))
    return rows, tag_group_columns
...
def export_workbook(
    ...
    *,
    ...
    rebuy_priority_columns: Optional[Sequence[str]] = None,
) -> Path:
    workbook = Workbook()
    ...
    rebuy_sheet = workbook.create_sheet("Rebuy")
    rebuy_columns = (
        build_column_order(
            rebuy_rows, extra_priority=rebuy_priority_columns
        )
        if rebuy_rows
        else list(COLUMN_ORDER_BASE)
    )
    write_sheet(rebuy_sheet, rebuy_rows, column_order=rebuy_columns)
    group_tag_columns(rebuy_sheet, rebuy_columns)
...
# ---------------------------------------------------------------------------
def main() -> None:
    parser = argparse.ArgumentParser(description="Retail data probe (Rebuy)")
    parser.add_argument(
        "--metafield",
        dest="metafields",
    ...
    output_path = export_workbook(
        ...
        rebuy_priority_columns=rebuy_tag_columns,
    )
