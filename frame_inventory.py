...
def format_price(value: Optional[int]) -> str:
    ...
    if not text:
        return ""
    try:
        # Coerce to float for consistent decimal formatting without scaling by 100.
...
def determine_jean_style(tags: Iterable[str]) -> str:
    ...
    for tag in tags:
        ...
        if normalized.startswith("filterleg::"):
            leg_value = tag.split("::", 1)[1].strip()
            norm_value = leg_value.lower()
            if norm_value == "pixie":
                pixie_present = True
                continue
            if norm_value and norm_value not in seen:
                seen.add(norm_value)
...
def write_csv(rows: List[Dict[str, Any]]) -> Path:
    ...
    output_path = OUTPUT_DIR / f"FRAME_{timestamp}.csv"
